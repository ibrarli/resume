...
export const ResumePreview: React.FC<ResumePreviewProps> = ({ data, theme }) => {
  ...
  return (
    <div className="relative flex flex-col items-center w-full px-2 sm:px-4 md:p-0 print:block print:p-0">
      ...
      {/* SCREEN ONLY: Page cutoffs hidden completely on mobile, shown only on desktop guides */}
      <div className="absolute inset-0 pointer-events-none hidden md:flex flex-col justify-start print:hidden mx-auto">
      ...
      {/* 
        ...
        - Uses 'w-full max-w-full md:w-[210mm]' for seamless on-screen auto-wrapping on phones.
        - Enforces 'print:w-[210mm] print:min-h-[297mm]' strictly for PDF engine rendering.
      */}
      <div className={`resume-canvas w-full max-w-full md:w-[210mm] md:min-h-[297mm] print:w-[210mm] print:min-h-[297mm] bg-white text-neutral-900 shadow-xl md:shadow-2xl p-5 sm:p-12 relative flex flex-col justify-between border border-neutral-200/60 print:shadow-none print:border-none print:p-0 ${themeClasses[theme]}`}>
        
        <div className={`flex-1 ${isSlateSidebar ? 'grid grid-cols-1 sm:grid-cols-[1fr_2.5fr] gap-5 sm:gap-8' : 'flex flex-col gap-5'}`}>
          
          {/* SIDEBAR BLOCK */}
          {isSlateSidebar && (
            <div className="bg-slate-50 p-4 rounded-xl print:bg-slate-100 flex flex-col gap-4 h-full">
              <div>
              ...
              <div className="flex flex-col gap-1.5 text-[12px] text-slate-600 break-all">
              ...
              {data.personalInfo.links.length > 0 && (
                <div className="flex flex-col gap-1.5 pt-3 border-t border-slate-200 text-[11px]">
                  {data.personalInfo.links.map((link) => link.platform && link.url && (
                    <a key={link.id} href={link.url.startsWith('http') ? link.url : `https://${link.url}`} target="_blank" rel="noreferrer" className="hover:underline text-slate-600 truncate block">
          ...
          <div className="w-full flex flex-col gap-5">
            {!isSlateSidebar && (
              <div 
                ...
                style={isColorful ? { marginLeft: '-1.25rem', marginRight: '-1.25rem' } : undefined}
              >
                <h1 className={`text-2xl sm:text-4xl font-bold tracking-tight uppercase m-0 ${
                ...
                <div className={`flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] sm:text-[13px] mt-2.5 ${isCentered ? 'justify-center' : 'justify-start'} ${isColorful ? 'text-slate-600' : 'text-neutral-500'}`}>
                  {data.personalInfo.location && <span>{data.personalInfo.location}</span>}
                  {data.personalInfo.email && (
                    <>
                      {data.personalInfo.location && <span className="hidden sm:inline">•</span>}
                      <a href={`mailto:${data.personalInfo.email}`} className="hover:underline font-medium print:no-underline break-all">{data.personalInfo.email}</a>
                    </>
                  )}
                  {data.personalInfo.phone && (
                    <>
                      {(data.personalInfo.location || data.personalInfo.email) && <span>•</span>}
                      <a href={`tel:${cleanPhone}`} className="hover:underline font-medium print:no-underline whitespace-nowrap">{data.personalInfo.phone}</a>
                    </>
                  )}
                  {data.personalInfo.website && (
                    <>
                      {(data.personalInfo.location || data.personalInfo.email || data.personalInfo.phone) && <span>•</span>}
                      <a href={data.personalInfo.website.startsWith('http') ? data.personalInfo.website : `https://${data.personalInfo.website}`} target="_blank" rel="noreferrer" className="hover:underline font-medium print:no-underline break-all">{data.personalInfo.website}</a>
                    </>
                  )}
                </div>

                {data.personalInfo.links.length > 0 && (
                  <div className={`flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] sm:text-[11.5px] mt-1.5 ${isCentered ? 'justify-center' : 'justify-start'}`}>
                    {data.personalInfo.links.map((link, idx) => link.platform && link.url && (
                      <React.Fragment key={link.id}>
                        ...
                        <a href={link.url.startsWith('http') ? link.url : `https://${link.url}`} target="_blank" rel="noreferrer" className="hover:underline font-medium print:no-underline text-neutral-500 break-all">
                ...
                <div className="space-y-3.5">
                  {section.items.map((item) => (
                    <div key={item.id} className="break-inside-avoid print:break-inside-avoid mt-2">
                      <div className="flex justify-between items-baseline font-bold text-neutral-800 flex-wrap sm:flex-nowrap gap-1">
                      ...
                      <div className="flex justify-between items-baseline text-neutral-600 font-medium text-[11.5px] sm:text-[12.5px] mt-0.5 flex-wrap sm:flex-nowrap gap-1">
